...
try:
    ...
    valid_complexity = set(data.get("rules", {}).get("valid_complexity", []))
    ...
    for f in data["features"]:
        if f["status"] not in valid:
            ...
            sys.exit(1)
        c = f.get("complexity")
        if c is not None and valid_complexity and c not in valid_complexity:
            print(f"[FAIL]  complexity inválida en feature {f['id']}: {c}")
            sys.exit(1)
...
echo ""
echo "── 2b. Validando models.config.json ────────────────────"

python3 - <<'PY'
import json, sys
try:
    cfg = json.load(open("models.config.json"))
    for key in ("tiers", "profiles"):
        if key not in cfg:
            print(f"[FAIL]  models.config.json sin clave '{key}'")
            sys.exit(1)
    for tier, platforms in cfg["tiers"].items():
        if not isinstance(platforms, dict) or not platforms:
            print(f"[FAIL]  tier '{tier}' inválido")
            sys.exit(1)
    valid_complexity = {"trivial", "simple", "medium", "complex", "very_complex"}
    for c, roles in cfg["profiles"].items():
        if c not in valid_complexity:
            print(f"[WARN]  profile complexity desconocida: {c}")
        for role in ("spec_author", "implementer", "reviewer", "explore"):
            if role not in roles:
                print(f"[FAIL]  profile '{c}' sin rol '{role}'")
                sys.exit(1)
    print("[OK]    models.config.json válido")
except FileNotFoundError:
    print("[FAIL]  Falta models.config.json")
    sys.exit(1)
except Exception as e:
    print(f"[FAIL]  models.config.json inválido: {e}")
    sys.exit(1)
PY
